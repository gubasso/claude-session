...
> Plan: cs-docs-hardening | Round: 2 of 4 | Complexity: L | Executor: prex (EF 1.5) | Generated: 2026-06-19 | Repo: /workspaces/claude-session
...
Reference (codex-session `doctor.rs` + the shell tool's `doctor` lesson, inspiration only): doctor must run subsystem resolvers with error capture rather than aborting in the global resolver; report structured results; a min-child-version check (codex used a `REQUIRED_..._VERSION` floor). Output discipline: human table → stderr or stdout per convention; `--json` machine output → stdout.
...
`assert_cmd`-test `doctor`/`doctor --json` against fixtures with one broken subsystem (verify it reports, does not abort, and exits non-zero appropriately).
