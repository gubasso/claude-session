...
Exit behaviour follows the inspection-versus-assertion split in `docs/reference/exit-codes.md`: `list` and `status` are inspection verbs and exit `0` whatever they find, including `list` with no accounts, `status` with nothing selected, and `status` reporting unusable authentication. Only a wrapper-side failure makes them non-zero.

`list` reports local-state usability and **never spawns the child**; `status` is the only subcommand that probes. A verb that does spawn the child keeps its own exit code and its own standard output rather than the child's, and attributes the child in the diagnostic and through the error document's `child_exit` (`docs/decisions/ADR-0068-spawn-the-child-as-a-subroutine.md`).
...
Removal confirms unless `--yes` is present, warns first when the account is selected, and reports plainly that deleting local state is not upstream revocation — unconditionally, in both modes, on standard error and never as a JSON field. Declining exits `0` and says nothing was removed.

**Removal takes the account's credential lock** before deleting anything, so it cannot interleave with a concurrent `account login`, and exits `LockBusy` past the deadline having changed nothing. It deletes `auth-mode.json` first as the commit, then the token, then the tree including the lock file itself, and clears the last-used marker only when that marker names the removed account (`docs/decisions/ADR-0069-destroy-the-credential-lock-with-its-scope.md`; inventory in `docs/reference/xdg-storage.md` § Cleanup and recovery). It looks for no running child.
...
Add the `commands/dispatch.rs` match arm. With `assert_cmd`, test each verb's JSON document against the field set in `docs/reference/accounts.md` § Reports, that `list` and `status` exit `0` on an empty, unselected, or unusable state, that `list` spawns no child, the `remove` confirmation and `--yes`, that `remove` under a held credential lock exits `LockBusy` and deletes nothing, and that no output stream ever carries token material.
...
- [ ] Each verb's `--json` is verb-level and carries exactly the fields `docs/reference/accounts.md` § Reports gives it; no credential appears in any output at any verbosity.
- [ ] `list` with no accounts, `status` with nothing selected, and `status` on unusable authentication all exit `0`, and `list` spawns no child.
- [ ] `remove` acquires the credential lock, exits `LockBusy` when it cannot, and leaves the config base, composed settings, the log, and every other account untouched.
