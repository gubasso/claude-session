...
- Run the project's own lint and test tasks before proposing changes. The gates are wired into `.pre-commit-config.yaml` — rustfmt, clippy, nextest, cargo-audit, cargo-deny, taplo, typos, gitleaks, ripsecrets, and cargo-machete — and `just hooks` is the single command that runs all of them, across both hook stages. `pre-commit run --all-files` is not enough on its own: it selects files, not stages, so it skips every push-stage hook. Continuous integration runs a subset of the gate, not the whole of it — see [testing and quality § The gate](./docs/reference/testing-and-quality.md#the-gate). Fix what the hooks report; do not bypass them.
