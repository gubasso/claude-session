...
[Milestones](./docs/plan/milestones.md) is the only delivery-status surface. Open the first `active` slice, or promote the first unblocked `shaped` slice, then load only the sources its `Governed by` section names. The slice entry is the execution contract; `tasks.md`, when present, only records cross-session checklist progress.
