...
- **Not blocked** — `token` mode, which injects a credential the child never refreshes; and any passthrough with no account selected, which the wrapper leaves alone under [ADR-0002](./0002-verbatim-argv-passthrough.md).
...
- Good: the revocation storm [ADR-0025](./0025-share-one-native-login-per-account.md) depends on avoiding cannot occur silently.
