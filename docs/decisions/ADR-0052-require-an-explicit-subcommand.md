...
Implying a subcommand leaves two spellings for one behavior, which ADR-0051 rejects. `account status [name]` accepts identifiers matching `[a-z0-9_-]{1,32}` ([XDG storage](../reference/xdg-storage.md#identifiers)), so `account list` becomes ambiguous with an account named `list`. `status` also fails with `Usage` on a first run while `list` exits `0`.
