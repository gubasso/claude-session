...
The file is written atomically after ingestion under [ADR-0027](./0027-ingest-secrets-only-from-stdin-or-a-terminal.md). Mode metadata records `mode`, `recorded_at`, and `sha256[..8]`; it never infers lifetime from a token prefix. Age and estimated expiry use the recorded or corrected mint time and label expiry as an estimate.
...
The wrapper never parses or fingerprints the child credential, speaks an OAuth endpoint, or manages API-billing credentials. A helper may replace file retrieval only through [ADR-0029](./0029-use-a-credential-helper-process-boundary.md).
