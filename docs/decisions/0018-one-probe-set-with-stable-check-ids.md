...
Identifiers are **public API**, on the same footing as `err.kind` in [ADR-0005](./0005-exit-code-taxonomy.md): scripts match them, so renaming one is a breaking change and the set is append-only in spirit. Each id maps to the `err.kind` that a failure of it exits with, which is what lets a guard's failure and `doctor`'s report describe one problem in one wording.
