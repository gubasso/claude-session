...
[ADR-0008](./0008-layered-error-architecture.md) fixed the error-type stack and [ADR-0005](./0005-exit-code-taxonomy.md) the code set, but nothing says how a typed error becomes a process status. It matters here because the wrapper must also reproduce a child's status including signal death, and because `std::process::exit` skips destructors — and the log's non-blocking file sink is flushed by one.
...
Extends [ADR-0008](./0008-layered-error-architecture.md), which fixed the error types but not their realization.
