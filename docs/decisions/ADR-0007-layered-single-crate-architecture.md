...
Amended by [ADR-0014](./ADR-0014-xtask-workspace-for-dev-tooling.md) — the second-binary trigger has fired, so the crate gains a library target and an `xtask` workspace member. The layer rules above are unchanged.
