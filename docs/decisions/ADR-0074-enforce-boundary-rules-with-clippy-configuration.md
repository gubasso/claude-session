...
Dependency direction and tooling isolation stay outside clippy. They are module-graph and manifest facts, and clippy has no lint for either. The manifest half of tooling isolation is additionally reachable through `cargo-deny`'s ban list.
...
Enacted by [`clippy.toml`](../../clippy.toml) and [`boundaries.rs`](../../tests/repo_contracts/boundaries.rs).
