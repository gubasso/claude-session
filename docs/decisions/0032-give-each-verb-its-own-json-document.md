...
[ADR-0024](./0024-machine-output-is-a-per-verb-flag.md) made `--json` verb-level but said nothing about what the document looks like. `doctor` needs a catalog with counts and a version; `version` needs two fields. Deciding this once matters because the shape is what scripts match on, and widening it later is a breaking change.
...
- Good: a verb's document can grow without an agreement across verbs, which is what ADR-0024 bought by making the flag verb-level.
