...
The predicate is the `open`: success prompts; failure exits `Unavailable` before side effects. A piped invocation still prompts.

Some tools gate prompting on standard input because prompts compete with piped data. No confirming verb here reads standard input, so a pipe carries no claim on the answer.
