...
Chosen option: a binding rule — YAGNI, stated in [`AGENTS.md` § Scope](../../AGENTS.md#scope).
...
The cost being avoided is not the writing. Every surface must fit the current slice's acceptance, be tested and documented, and keep working across later passthrough and XDG changes. A pre-implementation project pays that contract cost before it pays for code.
