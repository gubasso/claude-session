...
Every durable rule this project depends on lived either inside an implementation-plan round file — an execution artifact, not a specification — or outside the repository, which [ADR-0001](./0001-self-containment.md) forbids. Writing them down needs a placement rule first, or documents accumulate wherever the author happened to be looking.
