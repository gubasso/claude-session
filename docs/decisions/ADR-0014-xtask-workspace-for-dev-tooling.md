...
[ADR-0013](./ADR-0013-generate-config-examples-from-types.md) requires a generator that reflects over the wrapper's configuration types. That generator needs a schema crate and a renderer, neither of which belongs in a binary users install, and it needs to import the very types it documents — which a binary-only crate cannot export.
