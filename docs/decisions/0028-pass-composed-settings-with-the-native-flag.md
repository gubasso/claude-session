...
One account must share a durable child configuration directory, while settings composition remains specific to a terminal group. The wrapper needs to deliver both without turning `CLAUDE_CONFIG_DIR` back into a per-group credential boundary or violating [ADR-0002](./0002-verbatim-argv-passthrough.md).
...
See [ADR-0010](./0010-compose-native-settings-from-declared-layers.md), [configuration](../reference/configuration.md), and [process runtime](../reference/process-runtime.md).
