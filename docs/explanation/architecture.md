...
The conversion has happened. `Cargo.toml` declares a two-member workspace, the crate carries a library target exporting the configuration key descriptor and the artifact renderers, and `xtask/` is a second, unpublished binary depending on it by path. The renderers live in the shipped crate rather than in `xtask` for one reason: package integration tests link the library, so freshness is proven by calling them rather than by shelling out to cargo.

`xtask` is development tooling and never shipped surface. It is invoked as `cargo xtask <chore>`, it is not installed, and it is not on the CLI grammar in [the CLI surface](../reference/cli-surface.md). Its dependencies live in its own manifest and never enter the binary a user installs; today it needs none beyond the shipped crate itself. Nothing in `xtask/` may be imported by the wrapper; the dependency runs one way, as everywhere else here.
