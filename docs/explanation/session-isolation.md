...
The wrapper shares the directory and stops. It never reads, parses, or ages a registration — the child owns that schema, and no delegation obligation covers consuming it ([ADR-0089](../decisions/ADR-0089-carry-a-child-owned-fact-only-against-an-obligation.md)). A launch that cannot derive the scope, or cannot place the link, degrades to the private registry a session would otherwise have, because awareness is additive and must never stop an exec.
