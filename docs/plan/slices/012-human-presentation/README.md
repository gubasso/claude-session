...
- When `NO_COLOR` is set, the wrapper shall write no escape byte on any stream.
...
Each named surface carries an escape byte under a terminal and none under `NO_COLOR`, a redirected stream, or JSON, the plain text is byte-identical either way, and `just hooks` is green.
