...
This is the only delivery-status surface. Rows run in execution order, so the first `active` row, or the first `shaped` row whose dependencies are `done`, is the work to pick up. The [development workflow](../guides/development-workflow.md#pick-up-current-work) owns that procedure and the status vocabulary; the [charter](./charter.md) owns the appetite unit.
...
| 012 | [human presentation](./slices/012-human-presentation/README.md)                           | active | 2 implementation sessions | 011           | Carries the colour decision 011 left discarded through to bytes. |
