...
Blocks: whether `.credentials.json` belongs in [child facts](../reference/child-facts.yaml), and whether the discovery gate should stop being blind to it.
...
Exit: ADR, decide whether observing and deleting an artifact is a carry the registry governs at all, or whether the registry is about names the wrapper puts in front of the child. Registering it means extending the literal list, which makes the gate enforce every future mention; deciding it is out of scope means saying so in the registry header, because its current silence reads as coverage.
