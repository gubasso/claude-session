...
The native-passthrough foundation now uses its reviewed command-line, error, diagnostics, serialization, configuration, path, and process dependencies. Secure session storage added the profile and digest dependencies to the shipped graph, since the first profile and the first composed-settings digest are the triggers those two names were waiting on. The CLI-artifacts rung added the two generator dependencies for the same reason, the `completion` and `man` verbs being the triggers those names were waiting on. Which crates and versions are present remains the manifest and lockfile's fact; later-slice candidates stay reviewed but absent until their triggers occur.
...
| `fs4`, `fd-lock`, `fslock` | `std::fs::File::lock`                            | All predate the standard library's own advisory lock, stable since 1.89 against an MSRV of 1.97 ([ADR-0060](../decisions/ADR-0060-lock-the-writes-that-are-not-derivable.md))                                                                                   |
| `cap-std`, `openat`        | Non-following metadata checks                    | Both exist to confine traversal against untrusted input, which [ADR-0061](../decisions/ADR-0061-protect-storage-from-accidental-local-drift.md) places out of scope; the checks the wrapper does run need no capability handle                                  |
...
| `ansi_term`                | `anstyle` and `anstream`                         | Unmaintained with no patched release under [RUSTSEC-2021-0139](https://rustsec.org/advisories/RUSTSEC-2021-0139.html)                                                                                                                                           |
| `termcolor`                | `anstyle` and `anstream`                         | Supported for existing code but maintenance-only for a new project; [Cargo migrated to `anstream`](https://github.com/rust-lang/cargo/issues/12627) for the same feature set and a simpler API                                                                  |
