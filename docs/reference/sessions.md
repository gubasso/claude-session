...
Every failure costs a row its name and nothing else: a missing link, a real `sessions` directory, a target outside the registry shape, an absent registry, an invalid record or name, and two registrations answering to one pair all leave the row named by its directory. A name is never truncated, and a registration never changes a verdict.
