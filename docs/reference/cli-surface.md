...
The passthrough, `help`, `version`, `doctor`, the whole `account` namespace — `login [name]` with `--token`, `--stdin`, and `--minted-at`, plus `list`, `status`, and `remove` with `--yes` — `completion <shell>`, `man`, and `profile` are implemented, and so is requested help for the `account` namespace, for the two generator verbs, and for `profile` — `account --help`, `account <subcommand> --help`, `completion --help`, `man --help`, `profile --help`, and the matching `help <verb>` spellings. Requested help for `doctor` and `version` is not yet a result; that gap is tracked as `Q-006` in [open questions](../plan/open-questions.md).
