...
- `main` returns `std::process::ExitCode`, and `std::process::exit` is not called — it skips destructors, and the non-blocking log sink is flushed by one. Exec'ing the child is the single departure, because an image replacement does not return and runs no destructor either; it lives in the entry point, after the flush, and nowhere else. Every code the process can produce is owned by one enum, hand-rolled rather than taken from a crate. See [ADR-0035](../decisions/ADR-0035-convert-the-typed-error-to-a-code-once.md), [ADR-0084](../decisions/ADR-0084-exec-the-child-instead-of-supervising-it.md), and [exit codes](./exit-codes.md).
