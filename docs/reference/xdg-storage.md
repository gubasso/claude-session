...
| Session child config     | State  | `.../sessions/<namespace>/<session>/.claude.json`    | Child, after the launch seeds its first-run answers                | `0600`                         | Until `session clean` or account removal            |
| Session plugin state     | State  | `.../sessions/<namespace>/<session>/plugins/`        | The launch, which copies the seed's state in; the child thereafter | `0700`                         | Until `session clean` or account removal            |
...
| Asset tree               | Data   | `assets/`                                            | User                                                               | User's own; never written      | Until changed                                       |
| Plugin seed              | Data   | `plugin-seed/`                                       | User                                                               | User's own; never written      | Until changed                                       |
