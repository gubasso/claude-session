...
| Session directory        | State  | `accounts/<account>/sessions/<namespace>/<terminal>/` | Child, after the launch creates the directory        | `0700`                         | Until `session clean` or account removal            |
| Session witness          | State  | `.../sessions/<namespace>/.<terminal>.witness.json`   | Session subsystem                                    | `0600`                         | Until `session clean` or account removal            |
| Session child config     | State  | `.../sessions/<namespace>/<terminal>/.claude.json`    | Child, with the two keys a launch seeds              | `0600`                         | Until `session clean` or account removal            |
| Shared projects link     | State  | `.../sessions/<namespace>/<terminal>/projects`        | Session subsystem                                    | Link; the kernel's own         | Until `session clean` or account removal            |
...
| Peer registry link       | State  | `.../sessions/<namespace>/<terminal>/sessions`        | Session subsystem                                    | Link; the kernel's own         | Until `session clean` or account removal            |
