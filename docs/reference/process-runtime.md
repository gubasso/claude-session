...
2. Remove every key whose bytes begin `CLAUDE_SESSION_`, matched ASCII case-sensitively. The prefix is the wrapper's own namespace, so the sweep takes wrapper inputs as well as internals: `CLAUDE_SESSION_CHILD_BIN` and `CLAUDE_SESSION_DEFAULT_PROFILE` are consumed at startup, and a nested wrapper must not re-read a stale one.
3. Set `CLAUDE_SESSION_REENTRY=1`, the recursion marker.
4. Set `CLAUDE_CONFIG_DIR` to this agent's session directory, and `CLAUDE_SECURESTORAGE_CONFIG_DIR` to the account config directory — only when an account is selected. The first selects the child state one agent owns; the second keeps the saved login one file for the whole account ([ADR-0113](../decisions/ADR-0113-key-a-session-to-its-running-agent.md), [ADR-0104](../decisions/ADR-0104-share-one-credential-store.md)). Both are absolute and already resolved: the child normalizes the store value but never canonicalizes or tilde-expands it, so a value it would have to interpret selects a store the wrapper did not prepare.
5. Set `CLAUDE_CODE_PLUGIN_SEED_DIR` to the [plugin seed](../guides/supplying-child-plugins.md) — only when an account is selected and the tree exists. The child locates a seeded marketplace by probing that directory rather than by trusting the paths recorded inside it, which is what lets one read-only tree serve every session ([ADR-0117](../decisions/ADR-0117-supply-plugins-from-a-read-only-seed.md)).
6. Set `CLAUDE_CODE_OAUTH_TOKEN` to the retrieved token — only in token mode, and only at [step 5 of the launch sequence](#the-exec).
7. Set `CLAUDE_CODE_SUBSCRIPTION_TYPE` to the [declared plan](./accounts.md#declared-subscription-plan) — only in token mode, and only when the account recorded one.

Steps 5 to 7 drop an ambient value of the name they set, and steps 5 and 7 drop it even when they set nothing. Each describes something the launch is selecting, so an inherited one would describe something the child is not being given: a foreign token, a plan an account that declared none would otherwise appear to have declared, or a plugin tree the wrapper did not choose.

Those three are the wrapper's only removals besides step 2, and steps 3 to 7 are its only additions. Everything else the user exported — ambient authentication, `PATH`, locale — reaches the child untouched, which is what makes the child a normal program.
