...
4. Set `CLAUDE_CONFIG_DIR` to this terminal's session directory, and `CLAUDE_SECURESTORAGE_CONFIG_DIR` to the account config directory — only when an account is selected. The first selects the child state one terminal owns; the second keeps the saved login one file for the whole account ([ADR-0102](../decisions/ADR-0102-key-child-state-by-terminal.md), [ADR-0104](../decisions/ADR-0104-share-one-credential-store.md)). Both are absolute and already resolved: the child normalizes the store value but never canonicalizes or tilde-expands it, so a value it would have to interpret selects a store the wrapper did not prepare.
