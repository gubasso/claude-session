...
| `session-terminal-derives`  | Session | Hard     | `Unavailable`        | A terminal or a session leader, inside the namespace that issued it and discriminated by this kernel's identity, named the directory this run's child state belongs in | The terminal this session belongs to     |
