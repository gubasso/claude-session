...
The `doctor` verb, its report, `--list`, and `--strict` are implemented over 19 checks.
...
| `account-launch-ready`      | Session | Soft     | `DataFormat`         | The selected account has recorded that claude's first-run setup is done                                        | The selected account's first run         |
...
| `account-launch-ready`      | A launch under the selected account meets the child's first-run setup instead of its prompt, which asks to sign in again even though this account already can.  | Run `claude-session-rs account login {account}` again to record that the child's first-run setup is done. Pass the same `--token` input if this is a token account: the bare form signs in through the browser and stores that mode instead. |
...
Summary
  ...
  19 checks: 12 passed, 1 warning, 6 not applicable.
...
```json
{
  ...
  "wrapper": {
    ...
    "summary": { "total": 19, "passed": 19, "warned": 0, "failed": 0, "skipped": 0, "hard_failures": 0, "exit": 0 }
  },
