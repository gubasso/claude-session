...
| --------------------------- | ------- | -------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| `base-dirs-resolve`         | Host    | Hard     | `Unavailable`        | `HOME` is an absolute path, and config and state resolve to absolute, usable paths                                                                                                 | Wrapper storage locations                |
...
| `session-assets-linked`     | Session | Soft     | `Config`             | The asset tree or the child's own skill directory holds at least one of the names claude reads from its configuration directory                                                    | Your own skills, agents, and rules       |
...
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `base-dirs-resolve`         | Path resolution refused, so the wrapper acts on none of the answer. It needs an absolute `HOME` and usable XDG bases together, and no check below this one runs without both. | Set `HOME` to an absolute path. Then set `XDG_CONFIG_HOME` and `XDG_STATE_HOME` to absolute paths, or unset them so the defaults apply. `HOME` comes first because it is required whether or not a base falls back to it.                                         |
...
| `session-assets-linked`     | An isolated configuration directory reaches none of the skills, agents, or rules you wrote, so claude starts without them.                                                    | Put the agents, rules, and other assets you want in every session under `{path}`. Moving an existing collection there is enough. Skills come from `{skills}` instead, which is the directory claude reads them from already, so an installer needs no new target. |
